// ============================================================================
// SubmitEventMatchDuprDialog — organizer-only modal to push a social-event
// match to DUPR. Cloned từ SubmitDuprDialog (club path) nhưng nhận
// SocialEventMatchRow + invalidate query key của social-event-matches.
//
// Hai-path design giống bản club:
//   1. Validation: mọi player có dupr_id, tỷ số hợp lệ, đã xác định
//      winning_team.
//   2. Payload preview: copy JSON cho DUPR dashboard.
//   3. Manual submit: paste matchCode → gọi mark_match_submitted_to_dupr
//      (RPC đã được mở rộng ở migration 20260526120100 để chấp nhận match
//      có social_event_id).
// ============================================================================
...
import type { SocialEventMatchRow } from "@/hooks/useSocialEventMatches";

interface Props {
  match: SocialEventMatchRow;
  eventId: string;
  open: boolean;
    ...
    <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: 8 }}>
      {rows.map((row, i) => (
        ...
        >
          {row.passed ? (
            <CheckCircle2
              style={{ width: 16, height: 16, color: "var(--tl-green)", flexShrink: 0, marginTop: 2 }}
            />
          ) : (
            <XCircle
              style={{ width: 16, height: 16, color: "var(--tl-live)", flexShrink: 0, marginTop: 2 }}
            />
          )}
...
interface MarkSubmittedError {
  code: string;
  message: string;
}

function toError(error: unknown): MarkSubmittedError {
...
export function SubmitEventMatchDuprDialog({ match, eventId, open, onOpenChange }: Props) {
  const { t } = useI18n();
  ...
  const payload = useMemo(() => {
    const formatDupr = match.format === "singles" ? "SINGLES" : "DOUBLES";
    ...
    return {
      identifier: `tph:event:${match.id}`,
      matchDate: match.played_at.slice(0, 10),
      location: "ThePickleHub Social Event",
      format: formatDupr,
      matchType: "SIDEOUT",
      event: "ThePickleHub Social Event",
      bracket: "",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div style={{ padding: "28px 26px 22px" }}>
          ...
          <p style={{ fontSize: 14, color: "var(--tl-fg-3)", margin: 0, lineHeight: 1.5 }}>
            {m.dialogDesc}
          </p>
        </div>
        ...
        <div style={{ padding: "0 26px" }}>
          <div style={{ ...sectionStyle, paddingTop: 0, borderTop: "none" }}>
            <div style={{ ...labelStyle, marginBottom: 12 }}>{m.validationHeading}</div>
          ...
          <div style={sectionStyle}>
            ...
            <p style={{ marginTop: 10, fontSize: 12, color: "var(--tl-fg-3)", lineHeight: 1.5 }}>
              {m.payloadHint}
            </p>
          </div>

          <div style={sectionStyle}>
            <div style={{ ...labelStyle, marginBottom: 12 }}>{m.manualHeading}</div>
            <p style={{ fontSize: 13, color: "var(--tl-fg-3)", marginBottom: 16, lineHeight: 1.6 }}>
              {m.manualDesc}
            </p>
            <label htmlFor="ev-dupr-code" style={{ ...labelStyle, display: "block", marginBottom: 6 }}>
              {m.matchCodeLabel}
            </label>
            <input
              id="ev-dupr-code"
        ...
        >
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              ...
              style={{
                opacity:
                  markSubmitted.isPending || duprCode.trim().length < 1 ? 0.6 : 1,
              }}
            >
              {markSubmitted.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
              {m.manualSubmitCta}
            </button>
          </div>
        </div>
