...
import { fetchDuprSeeds } from "@/lib/dupr/seedFromDupr";
import { RoundFairnessCard, MexicanoCoverageBanner } from "@/components/matchmaking/RoundFairnessCard";
...
export default function SocialEventMatchmaking() {
  ...
  // Sprint C3 — organizer-facing toggle. Default ON so DUPR-balanced
  // pairing kicks in automatically when ≥75% of selected players have
  // a rating; falls back to legacy random pairing silently below that
  // threshold (with a banner).
  const [preferBalanced, setPreferBalanced] = useState(true);
  // Set when handleGenerate runs with preferBalanced=true but coverage
  // was too low — drives the MexicanoCoverageBanner.
  const [balancedRequestedButLow, setBalancedRequestedButLow] = useState(false);
  ...
  async function handleGenerate() {
    ...
    // Sprint C3 — pull DUPR doubles ratings for selected profiles and
    // prefer them over self_rated_level. Falls back per-player to
    // self-reported when DUPR is missing.
    const profileIds = chosen
      .map((r) => r.profile_id)
      .filter((id): id is string => !!id);
    let duprMap = new Map<string, { rating: number | null; syncedAt: string | null }>();
    try {
      duprMap = await fetchDuprSeeds(profileIds, "doubles");
    } catch (err) {
      console.warn("matchmaking: fetchDuprSeeds failed; using self-rated only", err);
    }

    const players = chosen.map((r) => {
      const dupr = r.profile_id ? duprMap.get(r.profile_id)?.rating ?? null : null;
      return {
        id: r.id,
        name: r.display_name,
        // Prefer DUPR over self-reported; falls back to self only when DUPR missing.
        level: dupr ?? r.self_rated_level ?? null,
      };
    });
    ...
    setSchedule(next);
    setBalancedRequestedButLow(preferBalanced && !next.balancedPairingApplied);
    ...
    <TheLineLayout title={`${mm.pageTitle} — ${eventTitle}`} active="events" noindex>
      <div className="tl-shell" style={{ paddingBottom: 60, maxWidth: 1080, margin: "0 auto" }}>
        ...
        <Card className="p-4 mb-4">
          ...
          {/* Sprint C3 — prefer DUPR-balanced pairing toggle */}
          {format === "mexicano" && (
            <div
              style={{
                marginTop: 12,
            ...
            >
              <Checkbox
                id="prefer-balanced"
                checked={preferBalanced}
                onCheckedChange={(c) => setPreferBalanced(!!c)}
              />
              <Label
                htmlFor="prefer-balanced"
                className="cursor-pointer"
                style={{ fontSize: 13, lineHeight: 1.4 }}
              >
                <div style={{ fontWeight: 500 }}>
                  {language === "vi"
                    ? "Ưu tiên cân bằng theo DUPR"
                    : "Prefer DUPR-balanced pairing"}
                </div>
                <div style={{ fontSize: 11, color: "var(--tl-fg-3)", marginTop: 2 }}>
                  {language === "vi"
                    ? "Auto kích hoạt khi ≥75% người chơi có DUPR. Tự fallback random khi thấp hơn."
                    : "Auto-enables when ≥75% of players have DUPR. Falls back to random below that."}
                </div>
              </Label>
            </div>
          )}
        ...
        {/* Sprint C4 — fairness card + low-coverage banner sit above the
            schedule itself so the organizer sees them before scrolling. */}
        {schedule && schedule.rounds.length > 0 && (
          <RoundFairnessCard schedule={schedule} />
        )}
        {schedule && schedule.rounds.length > 0 && (
          <MexicanoCoverageBanner
            schedule={schedule}
            requestedButFallback={balancedRequestedButLow}
          />
        )}
