...
function buildSeo(match: MatchDetail, language: "vi" | "en") {
  ...
  const fmtLabelVI =
    match.format === "singles" ? "Đơn" : match.format === "mixed" ? "Đôi nam-nữ" : "Đôi";
  const fmtLabelEN =
    match.format === "singles" ? "Singles" : match.format === "mixed" ? "Mixed" : "Doubles";
  const description =
    language === "vi"
...
function applyClientSeo(match: MatchDetail, language: "vi" | "en") {
  const { title, description, jsonLd } = buildSeo(match, language);
...
const MatchPage = () => {
  ...
  useEffect(() => {
    if (match) applyClientSeo(match, language);
    return () => {
      const existing = document.getElementById(JSONLD_ID);
      if (existing) existing.remove();
    };
  ...
  if (isLoading) {
    return (
      <TheLineLayout
        title={language === "vi" ? "Đang tải trận đấu" : "Loading match"}
        noindex
      >
  ...
  if (error || !match) {
    const notFoundTitle =
      language === "vi" ? "Không tìm thấy trận đấu" : "Match not found";
    const notFoundDesc =
      language === "vi"
        ? "Trận này có thể đã bị xóa hoặc URL không đúng."
        : "This match may have been deleted or the URL is incorrect.";
    return (
      <TheLineLayout title={notFoundTitle} description={notFoundDesc} noindex>
        <div className="mx-auto flex min-h-[60vh] w-full max-w-2xl flex-col items-center justify-center gap-4 p-8 text-center">
          <h1 className="text-2xl font-bold">{notFoundTitle}</h1>
          <p className="text-sm text-muted-foreground">{notFoundDesc}</p>
  ...
  const seo = buildSeo(match, language);
