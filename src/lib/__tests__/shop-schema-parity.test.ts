...
/** Objects created in the phase-1 migration (later migrations add more). */
const PHASE1_TABLES = SHOP_TABLES.filter((t) => t !== "products") as readonly (typeof SHOP_TABLES[number])[];
const PHASE1_VIEWS = ["my_shop_application"] as const;
...
describe("shop schema parity with migration 20260811090000", () => {
  ...
  it.each(PHASE1_VIEWS)("migration creates view %s", (view) => {
