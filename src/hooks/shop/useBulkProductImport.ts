...
import type { ProductInsert } from "@/integrations/supabase/shop-schema";
...
export function useBulkProductImport() {
  ...
  const publishBatch = useCallback(async (): Promise<PublishResult> => {
    ...
    if (approved.some((r) => !(r.aiData?.name || r.name).trim() || !(r.categoryOverride ?? r.aiData?.category)?.trim())) {
      throw new Error("invalid_product_data");
    }

    const batchId = crypto.randomUUID();

    const inserts: ProductInsert[] = approved.map((row) => ({
      shop_id: currentShopId,
      slug: uniqueSlug(row.aiData!.name || row.name, row.rowId),
      ...
      specs: sanitizeSpecs({
        ...
        ...(row.priceOverride
          ? { import_price_vnd: String(row.priceOverride) }
          : {}),
      }),
    ...
    setPublishing(true);
    try {
      // A single PostgREST insert is one SQL statement: all rows succeed or none do.
      const { data, error } = await shopFrom<ShopProductRow>("products")
        .insert(inserts)
        .select("*");
      ...
      setRows([]);
      return { count: inserts.length, batchId };
    } finally {
      setPublishing(false);
    }
...
function uniqueSlug(title: string, rowId: string): string {
  const base = title
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    ...
    .trim()
    .replace(/[\s-]+/g, "-")
    .slice(0, 180) || "san-pham";
  return `${base}-${rowId.replace(/-/g, "").slice(0, 8)}`;
}
...
function sanitizeSpecs(input: Record<string, unknown>): Record<string, string> {
  const output: Record<string, string> = {};
  for (const [rawKey, rawValue] of Object.entries(input)) {
    const key = rawKey.toLowerCase().normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/đ/g, "d")
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/^_+|_+$/g, "")
    ...
    if (!key || !value || value.length > 120 || output[key] !== undefined) continue;
    output[key] = value;
