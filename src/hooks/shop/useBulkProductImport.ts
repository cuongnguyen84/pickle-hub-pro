...
export function useBulkProductImport() {
  ...
  const publishBatch = useCallback(async (): Promise<number | null> => {
    ...
    setPublishing(true);
    try {
      const { error } = await shopFrom<ProductBulkImportInsert>("products")
      ...
      for (let i = 1; i < inserts.length; i++) {
        await shopFrom<ProductBulkImportInsert>("products").insert(inserts[i] as unknown as Record<string, unknown>);
      }
