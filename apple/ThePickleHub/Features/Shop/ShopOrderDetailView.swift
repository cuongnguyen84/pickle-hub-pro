...
final class ShopOrdersViewModel {
    ...
    func load() async {
        ...
        do { orders = try await repository.orders() }
...
struct ShopOrdersView: View {
    ...
    init(repository: any ShopOrderRepository = SupabaseShopOrderRepository()) {
        _model = State(initialValue: ShopOrdersViewModel(repository: repository))
    }
        ...
        card {
            ...
            if info.confirmedAt != nil {
                ...
                if info.gateway?.enabled == true {
                    Label("SePay tự động đối soát giao dịch", systemImage: "checkmark.shield.fill")
                        .font(TLType.titleSans(13)).foregroundStyle(TLColor.accentText)
                    if info.gateway?.status == "initiated" {
                        Label("Hệ thống đang kiểm tra nếu anh/chị vừa thanh toán.", systemImage: "clock")
                    ...
                    if let gatewayPayment = model.gatewayPayment {
                        ...
                        Button("Tải lại mã SePay") { Task { await model.startSePayCheckout() } }
