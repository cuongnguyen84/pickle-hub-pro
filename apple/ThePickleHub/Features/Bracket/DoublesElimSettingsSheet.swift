...
/// Creator-only Doubles Elimination management — rename, referee management,
/// delete. Mirrors TeamMatchSettingsSheet (DE has no separate "start" step; the
/// registration_open → ongoing transition is the "Đóng đăng ký" button).
@Observable
final class DESettingsModel {
    let tournament: DETournament
    var name: String
    var referees: [DEReferee] = []
    var newEmail = ""
    var busy = false
    var message: String?
    ...
    @MainActor func remove(_ ref: DEReferee) async {
        busy = true; message = nil
...
struct DoublesElimSettingsSheet: View {
    let tournament: DETournament
    let onChanged: () -> Void
    let onDeleted: () -> Void

    @Environment(\.dismiss) private var dismiss
    @State private var model: DESettingsModel
    @State private var confirmDelete = false

    init(tournament: DETournament, onChanged: @escaping () -> Void, onDeleted: @escaping () -> Void) {
        self.tournament = tournament; self.onChanged = onChanged; self.onDeleted = onDeleted
        _model = State(initialValue: DESettingsModel(tournament: tournament))
    }
