...
6. **Native app** — the SwiftUI app in `/apple` (Capacitor retired 2026-08-24, so the web bundle now only ever runs in a browser). Does the change break a contract the native app depends on — an RPC shape, a deep-link route, `push_tokens`? Native regressions ship through app-store review; there is no revert button.
