...
// ─── case 3 (pre-midnight form) — submit is closed while v1 is not effective ─
// The acceptance-gate form of this case runs after 17:00Z. Until then the
// server refuses for a DIFFERENT reason, and asserting that is worth doing:
// it is the one moment the "not published yet" branch is reachable at all.
{
  const s = await session("seller");
  const hasAccess = await rpc("shop_pilot_has_access", s.token, {});
  const submit = await rpc("shop_application_submit", s.token, { _expected_rules_version: "v1" });
  const legal = await sql(`SELECT (effective_at <= now()) AS eff FROM public.legal_documents WHERE document_key='seller-rules';`);
  const effective = legal.at(-1).eff;

  const detail = `pilot_access=${hasAccess.body.trim()} · effective_now=${effective} · submit HTTP ${submit.status} ${submit.body.slice(0, 140)}`;
  if (!effective && /seller_rules_not_published/.test(submit.body)) {
    record("3-pre", "submit refused while v1 is approved but not yet effective", "PASS", detail);
  } else if (!effective) {
    record("3-pre", "submit refused while v1 is approved but not yet effective", "FAIL", detail);
  } else {
    record("3-pre", "skipped — v1 is already effective, run the acceptance-gate form instead", "PASS", detail);
  }
}
