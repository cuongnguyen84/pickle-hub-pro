-- Create function to check user's quick table quota (max 3 per user in soft launch)
CREATE OR REPLACE FUNCTION public.get_user_quick_table_count(_user_id uuid)
RETURNS integer
LANGUAGE sql
...
-- Create function to check if user can create more quick tables
CREATE OR REPLACE FUNCTION public.can_create_quick_table(_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
-- Create RPC function to create a quick table with quota enforcement
CREATE OR REPLACE FUNCTION public.create_quick_table_with_quota(
  _name text,
  _player_count integer,
  _format public.quick_table_format,
  _group_count integer DEFAULT NULL,
  ...
  _registration_message text DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Generate share_id
  _share_id := encode(gen_random_bytes(6), 'hex');
