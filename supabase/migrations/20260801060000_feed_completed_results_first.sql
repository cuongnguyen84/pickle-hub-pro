-- Keep the mixed feed focused on published results rather than future fixtures.
-- Pro-tour rows rank by the moment a result is ingested/verified, while the
-- returned published_at remains played_at so cards still display match time.
...
RETURNS TABLE (
  item_type TEXT, item_id UUID, published_at TIMESTAMPTZ, ranked_at TIMESTAMPTZ,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  match_rows AS (
    SELECT 'match'::TEXT AS item_type, m.id AS item_id, m.played_at AS published_at,
      ...
      CASE
        WHEN m.source_provider IN ('ppa_tour', 'mlp', 'app_tour')
          THEN COALESCE(m.verified_at, m.updated_at, m.played_at)
        ELSE m.played_at
      END AS _ranking_at
    ...
    WHERE m.is_public=TRUE
      AND m.verification_status IN ('verified','disputed')
      ...
      AND CASE
        WHEN m.source_provider IN ('ppa_tour', 'mlp', 'app_tour')
          THEN COALESCE(m.verified_at, m.updated_at, m.played_at)
        ELSE m.played_at
      END >= (SELECT ts FROM window_start)
  ),
  blog_rows AS (
  ...
  video_rows AS (
    SELECT 'video'::TEXT, vid.id, vid.published_at, NULL::TEXT,
      ...
      'video:' || vid.id::TEXT, vid.published_at
  ...
  scored_rows AS (
    SELECT *, (
      EXP(-EXTRACT(EPOCH FROM (NOW()-_ranking_at))/3600.0/48.0)
  ...
  SELECT item_type, item_id, published_at, _ranking_at, _final_score, slug, format, match_type,
