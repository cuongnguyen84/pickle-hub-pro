...
type RegistryRow = { function_slug: string; display_name: string };
...
async function probe(slug: string): Promise<{ state: ProbeState; status: number | null; ms: number; reason: string }> {
  const started = performance.now();
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 8000);
  try {
    const response = await fetch(`${baseUrl}/functions/v1/${slug}`, { method: "OPTIONS", signal: controller.signal });
    ...
    if (response.status === 404 && /NOT_FOUND_FUNCTION_BLOB|Requested function was not found/i.test(body)) {
...
Deno.serve(async (req) => {
  ...
  const { data: registry, error } = await supabase.from("ops_edge_function_registry")
    .select("function_slug,display_name").eq("enabled", true).order("function_slug");
  if (error) return Response.json({ error: error.message }, { status: 500 });

  const results = [];
  for (const fn of (registry ?? []) as RegistryRow[]) {
    const result = await probe(fn.function_slug);
