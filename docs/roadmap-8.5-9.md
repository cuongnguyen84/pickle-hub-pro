...
| QA-04 | partial | 5d | Add stable E2E coverage for ten critical journeys — **stabilize DONE 2026-07-22 (#432)**; **inc4 SHIPPED 2026-07-22**: `tests/journeys.spec.ts` (J1-J10, non-mutating — registration journeys stop before the write, organizer journey rides the localStorage-only UX-04 autosave; single-worker, live-data discovery shared with a11y via `tests/helpers/discover.ts`). J4 immediately caught a real prod bug: TheLineLayout sign-in links dropped `?redirect` (fixed same PR). **Remaining to flip done:** Cuong sets repo secret `PLAYWRIGHT_ORGANIZER_CLUB_SLUG` (see `docs/huong-dan-viec-cua-cuong.md` §1) so J5/J6 stop self-skipping, then one green run | BASE-04, QA-03 |
...
Remaining open work: **QA-04** (stabilize done #432 — ten journeys remain) · **OPS-04** (inc1 done #432 — burn-alert deferred) · **PERF-05** (milestone fires 24/07, predicate satisfied ~27/07; dated milestones now live in docs/milestones.md + Telegram dispatcher) · **UX-07** partial — continue-or-close on the ~02/08 funnel read (D5 unmeasured) · **CLOSE-03/04** (now nearly satisfiable) · blocked: BASE-07 (participants), UX-09 (participants), OPS-02 (backup access), A11Y-05 (devices).
