...
| BE-01 | done | 2d | Edge Function CORS/serve sweep: move `corsHeaders` into `_shared/cors.ts` (CORS is not auth — don't grow `auth.ts` into a grab-bag), preserve all 13 characterized policy variants, and point all 37 inline copies plus the 35 former auth re-exports at it; migrate the six actual legacy std HTTP server imports (five `std@0.168`, one `std@0.190`) to `Deno.serve`, leaving all 76 handlers on one entrypoint API | SEC-04 |
...
| PERF-06 | done | 2d | Lazy-load locale dictionaries through `I18nProvider`; the initial route requests only its active language and locale chunks stay out of the PWA precache. Entry gzip fell by 67,225 bytes (39.2%); initial JS including the requested dictionary fell by 18.7–20.0% | — |
...
| 2026-07-15 | PERF-06 | Source commits `b8ca5688` + `99755138`; GitHub Actions runs `29403335011`, `29403334899`, `29403334889`, and `29403334116` | Dynamic imports and promise caching isolate `en`/`vi`; switching is atomic, failed imports can retry, stale geo responses cannot override a user choice, and `/vi` wins over geo detection. `ANALYZE=1` entry moved from 590,500 raw / 171,641 gzip bytes to 364,207 / 104,416, a 67,225-byte (39.2%) entry reduction. Initial JS plus the active dictionary is 137,239 gzip bytes for English or 139,567 for Vietnamese, saving 18.7–20.0%; neither HTML nor the service-worker precache references locale chunks. Production Chrome confirmed `/` loads only `locale-en` with `lang=en` and `/vi` loads only `locale-vi` with `lang=vi`, all JavaScript responses 200 and no page errors. CI passed the 1,950 KB bundle budget at 1,949.3 KB, 651 tests/10 skipped across 47 files with 83.96% statement coverage, typecheck, changed-file lint/TheLine, 276-migration duplicate guard, build, Playwright smoke, npm audit, CodeQL, and deploy guard. Rollback: revert both source commits together; no data or migration rollback is required. |
...
Last updated: 2026-07-15 after completing BE-01.

- Active task: none. `HOT-01`..`HOT-04`, `HOT-06`, `HOT-07`, `BASE-01`, minimum-scope `BASE-03`, `BASE-06`, `OPS-00`, `SEC-02`, `SEC-03`, `SEC-04`, `BE-01`, `QA-06`, and `PERF-06` are complete.
- Next task: `BE-02`; fix admin push broadcast at the root by resolving recipients server-side, batching FCM work with bounded `Promise.allSettled` chunks, pruning tokens reported as unregistered, and adding the missing destructive-action confirmation.
