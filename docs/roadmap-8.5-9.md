...
| DS-03 | done | 5d | Standardize Button, IconButton, Input, Select, Card, Badge, Dialog, and Sheet. **Shipped 2026-07-18** (#403, proposal ds-03-component-standardization qua /idea 2 vòng panel): Button + variant `tl-primary` (kem) theo bảng map D4 (green→default, base→outline); 44px touch targets (default h-11, icon 44×44, lg h-12) — phần Button của A11Y-02 đóng luôn; 4 journey screens rời `.tl-btn`; ratchet changed-files trong check-theline (**HARD từ 2026-08-03, PR #534** — kèm fix rename→before=0 false-positive bằng --find-renames); native TLButton(.green/.cream/.outline) + TLIconButton(label bắt buộc)/TLBadge/TLSelect/TLSheet(ScrollView)/TLDialog + TLComponentsRenderTests chạy AX3 trong CI (bộ UI test native đầu tiên). App Store submit vẫn RED-gated chờ checklist test tay | DS-02 |
...
| A11Y-05 | done | 3d | Run VoiceOver, Dynamic Type, keyboard-only, and contrast manual audit — **DONE 2026-07-22**: Cuong hoàn tất audit tay (URL audit `/social/co-dinh-thu-2` + `/clb/test`) | DS-03, test devices |
...
| UX-09 | deferred | 3d | Repeat usability sessions and calculate task success, time, and SUS — **CLOSED 2026-07-22 by Cuong**: bỏ formal sessions, cùng lý do BASE-07 (informal đủ). Template `docs/usability-baseline-2026-07.md` vẫn dùng được nếu sau này cần | UX-01..UX-08, participants |
...
| QA-04 | done | 5d | Add stable E2E coverage for ten critical journeys — **stabilize DONE 2026-07-22 (#432)**; **inc4 SHIPPED 2026-07-22**: `tests/journeys.spec.ts` (J1-J10, non-mutating — registration journeys stop before the write, organizer journey rides the localStorage-only UX-04 autosave; single-worker, live-data discovery shared with a11y via `tests/helpers/discover.ts`). J4 immediately caught a real prod bug: TheLineLayout sign-in links dropped `?redirect` (fixed same PR). **DONE 2026-07-22:** secret `PLAYWRIGHT_ORGANIZER_CLUB_SLUG` set, smoke re-run [29900709731](https://github.com/cuongnguyen84/pickle-hub-pro/actions/runs/29900709731) green with J5/J6 passed (53 passed) | BASE-04, QA-03 |
...
| PERF-05 | done | 3d | Validate CWV p75 targets with Vietnam field data. **Read 2026-07-28, verdict GIỮ** (docs/perf-05-report-2026-07-28.md): LCP p75 mobile 2684→2423ms crossed into good after #389+#417; CLS p75 ~0.67 POOR pre-existing both windows (NOT a regression); INP no CrUX sample. **CLS work since: #502 attribution, #504 feed, #515 CWV measurement + layout stability — field re-read ≥30/08 (docs/seo-followup-2026-08.md item 5).** GA4 gap found+fixed: metric_name/metric_rating were never registered (21/07 batch was funnel/badge only) — registered 28/07, follow-up read = milestone PERF-05B 2026-08-05 | PERF-02..PERF-04 |
...
| OPS-04 | done | 4d | Build actionable dashboards and alerts tied to SLOs — **inc1 DONE 2026-07-22 (#432)**: uptime-ping (SLO-1 gap) + milestone-due watchdog + alert_count fix; **inc2 02-03/08: #525 AdminJobs dashboard + Telegram morning digest + self-heal, #528 alert dedupe.** **inc3 DONE 2026-08-03 (PR #535, `72e2cbdb`)**: P1 volume alert độc lập fingerprint (≥25/60min, bắn cả ban đêm) + P2 burn-rate 24h vs budget 30d (3000/30d, vào 2×/ra 1× hysteresis, night-quiet 22-07 ICT re-fire sau 07:00) + transition dedup + recovery bắt buộc (`ops_slo_burn_state`); 13 unit test; deployed + migration applied prod 03/08 | OPS-03, BASE-03 |
...
- SEO metadata is generated from one manifest.
  **SEO closed 2026-07-26**: `SEO-01`..`SEO-04` done, plus the "bracket
  generator" cluster (`docs/seo-tools-cluster-intent-map.md`, now CLOSED) and
  the two systemic defects it surfaced — EN posts served Googlebot ~0.9 KB of
  body (#467) and two thirds of SERP titles shipped truncated by the 60-byte
  budget (#468). Outcome read is milestone `2026-08-23 SEO-CLUSTER-READ`.
  **Post-close SEO continued as its own track:** #503 canonical guard, 410/301
  sweep (30/07), #530 SEO CI guards + index-coverage classifier, #533 VI venue
  links + byte-safe titles — live checklist `docs/seo-followup-2026-08.md`
  (items 2-5 open).
...
| CLOSE-03 | done | 3d | Run final UX, accessibility, performance, security, and codebase audit | All phases |
| CLOSE-04 | done | 1d | Publish scorecard with evidence and remaining exceptions. **Done 2026-08-03:** `docs/roadmap-8.5-9-scorecard.md` — 7 phase chấm theo đúng exit criteria, 5 ngoại lệ đánh số (Lighthouse a11y không số, blanket contrast, Phase-3 metrics unmeasured-by-decision, CLS chờ 30/08, 8 nợ audit) | CLOSE-03 |
...
| 2026-07-26 | SEO cluster Sprint 1 | PRs #449, #462, #466, #465 | Bracket-generator cluster deduped: templates post merged into the how-to guide (EN→EN + VI→VI 301s), round robin guide re-angled informational, organizer hub made a real pillar, `/tools` retitled to the head term with FAQ + FAQPage schema. Audit-safe throughout — no URL is both a 301 and a 200. Also closed two leaks step 1 missed: the VI transactional twin with no `alternate_en_slug`, and 11 dead VI internal links |
| 2026-07-26 | EN blog body on the bot path | PR #467 | `renderBlogPost` emitted breadcrumb + related links only: every EN post served Googlebot ~0.9 KB of `<main>` while the VI twins served 8.4 KB. Now renders the post's own sections + FAQ + `FAQPage`/`HowTo` schema (0.9 KB → 6.3-17.7 KB). Barrel exports lazy loaders, not values, because `_middleware.ts` runs for every request — 141 esbuild `__esm()` wrappers; bundle 220 KB → 585 KB gz against a 3 MB floor; prod bot TTFB 164 ms |
| 2026-07-26 | SERP byte budget | PR #468 | 63/92 titles + 55/92 descriptions in `metadata.ts` and 39+50 rows in `vi_blog_posts` shipped ellipsised. Root cause was not just copy: `buildTitle` decided the brand suffix fit by counting CHARACTERS against a 60-BYTE budget. All 118 strings rewritten, function made byte-aware, post-file ↔ metadata drift (5 titles, ~30 descriptions) fixed. Guards: hard-gate test, `buildTitle` unit tests, and CHECK constraints on `vi_blog_posts` (migration `20260726120000`, mutation-verified 23514). Prod scan of 98 blog URLs: 0 truncated |
...
Last updated: **2026-08-03** after SEO checklist v2 (#530), VI venue links + byte titles (#533), TheLine Rule 4 HARD (#534), UX-07 funnel read → CLOSE (`31a71f54`), weekly /san tracker (SEO-SAN-W33). Counts: **71 done · 0 partial · 0 later · 3 closed-by-decision (BASE-07, UX-07, UX-09) · 0 blocked — ROADMAP ĐÓNG SỔ 03/08/2026**, scorecard: `docs/roadmap-8.5-9-scorecard.md`, kế tiếp: `docs/roadmap-9.md` (A11Y-05 done 22/07 tay — note "blocked: devices" cũ tự mâu thuẫn).

Remaining open work: **none** — roadmap này đóng. Việc tiếp theo sống ở `docs/roadmap-9.md` + `docs/milestones.md` + `docs/seo-followup-2026-08.md`.

Debt not tracked as rows, arguably higher priority than some `later` rows — see `docs/roadmap-status-2026-07-21.md` §"Open risks": (1) **RED approval gate unenforceable** — pipeline writes to GitHub as Cuong; release-pilot now hands off instead of merging RED; real fix = separate bot identity + fine-grained PAT — **DONE: bot `thepicklehubnet` live 21/07**; (2) **a11y page-wide contrast still disabled** in the axe suite — real debt behind the blanket rule, CLOSE-03 should quantify; (3) Lighthouse "green" is timing-dependent, not proof; (4) team-match payment may be dead not just unused (1/15 fee tournaments, 0 teams). Older loose ends still open (ratchet HARD shipped #534): manual admin push verify, gen Supabase types (`--schema public`), SEC-06 ledger reconciliation, `docs/manual-test-backlog.md` items 8–14.
