...
| ARCH-02 | doing | 5d | Refactor Social Event registration/payment into domain/application/infrastructure/UI layers. **Increment 0 done** (DB-01c #384) · **increment 1 done 2026-07-18** (#397: cancel/reactivate-registration → Deno-free handler.ts + 16 vitest pinning the full status+code contract). Next: increment 2 = characterization tests for RegistrationModal money path (HARD GATE before touching the component; needs @testing-library/jsdom infra — repo has none), then extraction/i18n per docs/proposals/arch-02-03-refactor. NOTE: putting tests under `_shared/__tests__/` triggers deploy-guard full-fleet redeploy (75 functions, succeeded 2026-07-18 but avoid: put future handler tests elsewhere or accept the fleet cost knowingly) | ARCH-01 |
