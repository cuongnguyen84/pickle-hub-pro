...
- Still open from that list: confirm the production `DUPR_CLIENT_KEY` format/entropy without printing it (fingerprint scheme unchanged until then); review raw `viewer_ip`/`user_id` retention claims; set `GITHUB_TOKEN` secret so cron-health stops skipping the GitHub monitor (optional, no false alerts meanwhile).
