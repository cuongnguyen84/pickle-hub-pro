...
| P9 | toàn bộ | 🔴 **Bản mẫu không cuộn được** khi Cuong mở lần đầu. `html`, `body` và `#root` đều `overflow: hidden; height: 100%` (`src/index.css`) nên document không bao giờ cuộn — mỗi trang phải tự dựng scroller. `/proto/shop` quên làm. **Bộ chụp ảnh che mất lỗi này**: nó phóng viewport bằng chiều cao nội dung trước khi chụp, nên 236 ảnh đều đẹp trong khi không trang nào cuộn được. | Đã sửa: `.tl-shop` thành vỏ cố định + `.tl-shop-scroll` là scroller (đúng khuôn `.tl-root`/`.tl-scroll` của `the-line.css`). Gate Q01 giờ **cuộn thật rồi đọc lại `scrollTop`** — bài học: chụp ảnh đẹp không chứng minh trang dùng được. Mọi trang mới trong repo này đều phải tự dựng scroller. |
| P10 | B04/B09/S06/A03 | 🔴 **Blocker demo:** `ChatFAB` đè lên nút chính của mọi thanh hành động dính ở 375px. | Đã ẩn trên `/proto/shop` + `/shop` + `/seller` + `/admin/shop`. Gate Q01 kiểm tra. |
| P11 | S0*/A0* | 🔴 **Blocker demo:** `BottomNav` toàn cục hiện cả trên Kênh người bán và Quản trị Shop — hai thanh chồng nhau ở đáy. | Đã ẩn (P2 dự đoán trước, giờ thực thi). Gate Q01 kiểm tra cả chiều ngược lại: trang mua hàng vẫn phải có BottomNav. |
| P12 | A02–A05 | **`.tl-admin-frame` có `overflow: hidden` + con grid `min-width: auto`** → bảng trong `.tl-admin-body` đẩy cột rộng hơn viewport rồi bị **cắt âm thầm**, không thanh cuộn, không tín hiệu. A02 mất 310px bảng ở 768px ngay từ đầu. | Đã thêm `min-width: 0`. Quan trọng hơn: phép đo tràn ngang giờ **so mép phải với hộp scroller** thay vì đọc `scrollWidth` — `scrollWidth` bị vô hiệu bởi bất kỳ `overflow:hidden` trung gian nào. Đây là lần thứ hai gate mù vì cùng một lý do. |
| P13 | B04 | Mỗi phiên bản đều có `mediaIndex` trong fixture nhưng trang sản phẩm **không dùng** — chọn "Đen" thì SKU/giá/tồn đổi còn ảnh vẫn là giày trắng. | Đã nối, khoá bằng `__tests__/variant-media.test.ts`. Ánh xạ theo **lựa chọn chưa đủ** để đổi màu là ảnh đổi ngay, không chờ chọn xong size. |
