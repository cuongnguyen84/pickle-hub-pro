...
export function renderTools(siteUrl: string, _rawPath = "/tools", lang: "en" | "vi" = "en"): Response {
  ...
  return htmlResponse(buildHtml({
    title: TOOLS_HUB_META[lang].title,
    description: TOOLS_HUB_META[lang].description,
    url: canonical,
    siteUrl,
      ...
      "@graph": [
        {
          ...
          description: isVi
            ? "Công cụ tạo bảng đấu pickleball miễn phí — vòng tròn tính điểm, loại trực tiếp đơn và đôi, đấu đồng đội MLP, định dạng linh hoạt. Chấm điểm trực tiếp, bảng điểm chia sẻ được. Đăng nhập để tạo giải."
            : "Free pickleball tournament bracket generator — round robin, single and double elimination, MLP team match, flex format. Live scoring, shareable scoreboard. Account required to create.",
...
<p>Trình tạo bảng đấu pickleball miễn phí — chia cặp vòng tròn tính điểm (round robin), loại trực tiếp đơn và đôi, đấu đồng đội kiểu MLP, và định dạng linh hoạt. Chấm điểm trực tiếp trên điện thoại, link bảng điểm chia sẻ được, bracket in được. Đăng nhập để tạo và quản lý giải. Xem bảng đấu công khai không cần tài khoản.</p>
...
<p>Đăng nhập để tạo giải; xem bảng đấu công khai không cần tài khoản. Không cần tải về. Không có kiểu dùng thử 14 ngày rồi thành gói $99/tháng. Xây dựng và duy trì bởi <a href="${siteUrl}/vi">ThePickleHub</a>, nền tảng pickleball song ngữ Việt-Anh đưa tin PPA Tour Asia, MLP và các giải pro khu vực.</p>
...
<p>A free pickleball tournament bracket generator — round robin, single and double elimination, MLP team match, and flex format. Live scoring on your phone, shareable scoreboard URL, printable bracket. Sign in to create and manage a tournament. Public brackets can be viewed without an account.</p>
...
<p>Create and manage tournaments with a free account. Spectators can view public brackets without signing in. Built and maintained by <a href="${siteUrl}/blog/tournament-organizer-hub">ThePickleHub</a>, a bilingual Vietnamese-English platform reporting on PPA Tour Asia, MLP, and the regional pro circuit.</p>
...
const TOOL_PAGE_META: Record<string, { title: string; description: string }> = {
  "quick-tables": {
    ...
    description: "Free round-robin & single elimination bracket generator. Auto-scheduling, real-time scoring, shareable links. Sign in to create; public viewing is free.",
  },
  "team-match": {
    title: "Team Match – MLP Format Pickleball | ThePickleHub",
    description: "Free MLP-style team match pickleball bracket tool. Manage team lineups, track singles and doubles results, and generate instant standings. Account required to create; public viewing needs no login.",
  },
  "doubles-elimination": {
    title: "Doubles Elimination Bracket Generator | ThePickleHub",
    description: "Free doubles elimination bracket generator. Auto bracket draw, live scoring, shareable results. Sign in to create; public viewing is free.",
  },
  "flex-tournament": {
...
const TOOL_NEW_PAGE_META: Record<string, {
  ...
  "flex-tournament": {
    en: {
      ...
      description: "Free flexible tournament generator. Custom groups, brackets, and match formats. Sign in to create; public viewing is free.",
    },
    vi: {
  ...
  "team-match": {
    en: {
      ...
      description: "Free MLP-style team match software. Lineup management, multi-round scoring, dreambreaker support. Sign in to create; public viewing is free.",
    },
    vi: {
